...
function crocodile_AddMenu(&$m){
	global $zbp;
	$m[]=MakeTopMenu("root",'鳄鱼主题配置',$zbp->host . "zb_users/theme/crocodile/option.php","","topmenu_crocodile");
}
...
function UninstallPlugin_crocodile(){
	global $zbp;
	//$zbp->DelConfig('metro');
}
...
function metro_savetofile($stylefile){
	...
	$strContent = @file_get_contents($zbp->usersdir . 'theme/crocodile/source/style.css.html');
